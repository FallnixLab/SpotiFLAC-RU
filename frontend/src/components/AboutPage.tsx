...
export function AboutPage() {
  ...
  const formatReleaseTimeAgo = (dateString: string): string => {
    ...
    if (totalYears > 0) {
      ...
      return remainingMonths > 0 ? `${totalYears} г. ${remainingMonths} мес. назад` : `${totalYears} г. назад`;
    }
    if (totalMonths > 0) {
      const remainingDays = totalDays % 30;
      return remainingDays > 0 ? `${totalMonths} мес. ${remainingDays} дн. назад` : `${totalMonths} мес. назад`;
    }
    if (totalDays > 0) {
      const remainingHours = totalHours % 24;
      return remainingHours > 0 ? `${totalDays} дн. ${remainingHours} ч. назад` : `${totalDays} дн. назад`;
    }
    if (totalHours > 0) {
      const remainingMinutes = totalMinutes % 60;
      return `${totalHours} ч. ${remainingMinutes} мин. назад`;
    }
  ...
  return (<div className="flex flex-col space-y-3">
    <div className="flex items-center justify-between shrink-0">
      <h2 className="text-2xl font-bold tracking-tight">О программе</h2>
    </div>
        ...
        <div className="grid gap-2 grid-cols-3">
            ...
            {repoStats["SpotiFLAC-Next"] && (<CardContent className={`${projectCardContentClass} space-y-2`}>
              ...
              <div className="rounded-md border border-sky-500/25 bg-sky-500/8 px-3 py-2">
                ...
                <p className="text-xs leading-snug text-sky-700 dark:text-sky-300">
                  Этот проект был создан в качестве благодарности всем, кто поддержал SpotiFLAC на Ko-fi.
                </p>
              </div>
            </CardContent>)}
          </Card>
          <Card className={projectCardClass} onClick={() => openExternal("https://github.com/afkarxyz/Twitter-X-Media-Batch-Downloader")}>
            ...
            {repoStats["Twitter-X-Media-Batch-Downloader"] && (<CardContent className={`${projectCardContentClass} space-y-2`}>
              ...
              <div className="flex flex-col gap-1 text-xs text-muted-foreground items-start">
                <span className="flex items-center gap-1">
                  <Download className="h-3.5 w-3.5" /> ВСЕГО:{" "}
                  {formatNumber(repoStats["Twitter-X-Media-Batch-Downloader"]
                    .totalDownloads)}
                </span>
                <span className="flex items-center gap-1 text-green-600 dark:text-green-400">
                  <Download className="h-3.5 w-3.5" /> ПОСЛЕДНЯЯ ВЕРСИЯ:{" "}
                  {formatNumber(repoStats["Twitter-X-Media-Batch-Downloader"]
                    .latestDownloads)}
                </span>
              </div>
          ...
          <div className="flex h-full flex-col gap-1.5">
            <Card className={`${projectCardClass} flex-1`} onClick={() => openExternal("https://exyezed.qzz.io/")}>
              <CardHeader className={projectCardHeaderClass}>
                <CardTitle className="leading-tight">Браузерные расширения и скрипты</CardTitle>
            ...
            <Card className={`${projectCardClass} flex-1`} onClick={() => openExternal("https://spotubedl.com/")}>
              <CardHeader className={projectCardHeaderClass}>
                ...
                <CardDescription className={projectBodyClass}>
                  Скачивайте треки, альбомы, плейлисты и дискографию Spotify в формате MP3/OGG/Opus.
                </CardDescription>
              </CardHeader>
            </Card>
          </div>
        ...
        <div className="flex flex-col md:flex-row w-full max-w-3xl bg-card rounded-xl border shadow-sm">
          ...
          <div className="flex-1 p-6 flex flex-col items-center justify-between border-b md:border-b-0 md:border-r space-y-6">
            <div className="flex flex-col items-center space-y-4">
              <div className="h-32 flex items-center justify-center w-full">
                <img src={KofiLogo} className="w-32 object-contain pointer-events-none" alt="DonationAlerts" />
              </div>
              <h4 className="font-semibold text-foreground">Поддержать через DonationAlerts</h4>
              <p className="text-sm text-muted-foreground text-center px-4">
                Нравится проект? Вы можете поддержать дальнейшую разработку через DonationAlerts.
              </p>
            </div>
            <Button className="h-10 w-full text-sm font-semibold text-white gap-2 group bg-[#ff7200] hover:bg-[#e66700]" onClick={() => openExternal("https://dalink.to/fallnix")}>
          ...
          <div className="flex-1 p-6 flex flex-col items-center justify-between space-y-6">
            <div className="flex flex-col items-center space-y-4 w-full">
              ...
              <h4 className="font-semibold text-foreground">USDT (TON)</h4>
              <p className="text-sm text-muted-foreground text-center px-4">
                Также принимаются крипто-пожертвования. Отсканируйте QR-код или скопируйте адрес.
              </p>
            </div>
            <div className="flex items-center gap-2 bg-muted/50 pl-3 pr-1.5 py-1.5 rounded-lg border w-full justify-between h-10">
              <code className="text-xs font-mono text-muted-foreground truncate" title="UQBxgfoz6qbGLsMF-eUIsXaD23d1Q9VbkhyXQZu662LFSiq7">
                UQBxgfoz6qbGLsMF-eUIsXaD23d1Q9VbkhyXQZu662LFSiq7
              </code>
              <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0 hover:bg-background" onClick={() => {
                navigator.clipboard.writeText("UQBxgfoz6qbGLsMF-eUIsXaD23d1Q9VbkhyXQZu662LFSiq7");
                setCopiedUsdt(true);
